...
function getPangkatUser(member){
  const pangkatRoles = MAPPING_ROLE.PANGKAT.map(r=>r.value).filter(id=>id !== '-');
  const role = member.roles.cache.find(r=>pangkatRoles.includes(r.id));
  return role ? role.id : '-';
}

async function handlePromotionReady(client){
  // mendaftarkan slash command (global). Bisa diubah jadi guild command jika perlu.
  try {
    ...
    console.log('✅ /promosi terdaftar');
  } catch (err) {
    console.error('Gagal mendaftar /promosi:', err);
  }
}

async function handlePromotionInteraction(interaction){
  try {
    // Chat input command /promosi
    if(interaction.isChatInputCommand() && interaction.commandName === 'promosi'){
      ...
      if(!member.roles.cache.some(role=>ROLE_ALLOWED_PROMOSI.includes(role.id))){
        return interaction.reply({ content:'❌ Anda tidak memiliki izin menggunakan command ini.', ephemeral:true });
      }
      ...
      await interaction.reply({ content:'### 📜 Form Pembuatan Surat', components:[row], ephemeral:true });
      return;
    }
    ...
    // only process select/user menus below
    if(!interaction.isStringSelectMenu() && !interaction.isUserSelectMenu()) return;

    const userData = dataSurat.get(interaction.user.id);
    if(!userData) return;

    const val = interaction.values ? interaction.values[0] : (interaction.users ? [...interaction.users.keys()][0] : null);
    ...
    switch(interaction.customId){
      ...
      case 's4':
        ...
        const rowTarget = new ActionRowBuilder().addComponents(
          new UserSelectMenuBuilder().setCustomId('s5').setPlaceholder('4. Pilih Nama Pihak Terkait')
        );
        await interaction.update({ content:'Pilih Pihak Terkait', components:[rowTarget] });
        break;

      case 's5':
        // val is user id (string)
    ...
    console.error('promotion handler error:', err);
    try { if(interaction && !interaction.replied) await interaction.reply({ content: 'Terjadi error saat memproses form.', ephemeral: true }); } catch(e){}
  }
}

async function showModal(interaction){
  const modal = new ModalBuilder().setCustomId('modal_pertimbangan').setTitle('Input Pertimbangan');
  const input = new TextInputBuilder().setCustomId('input_pertimbangan').setLabel('Masukkan Pertimbangan').setStyle(TextInputStyle.Paragraph);
...
async function showStep(interaction,customId,placeholder,options){
  ...
  await interaction.update({ content:`Mengisi: **${placeholder}**`, components:[row] });
}
...
<@&1496865881672912899>`;
  ...
  await interaction.reply({ content:'✅ Surat berhasil diterbitkan', ephemeral:true });

  const channel = interaction.guild.channels.cache.get(CHANNEL_PROMOSI);
  if(channel) channel.send(surat);

  dataSurat.delete(interaction.user.id);
}

module.exports = {
  handlePromotionReady,
